...
class PPIAbstractClass:
    def __init__(self, inputs_gt, inputs_hat, inputs_unl, lambd_mode="overall"):
    ...
    def get_asymptotic_distribution(self):
        # lambd_ =
        # if lambd_ is None:
        #     lambd_ = self.get_lambda(idx_to_optimize=idx_to_optimize)
        # print(f"lambda: {lambd_}")
        # needs_theta_fit = self.theta is None
        # if needs_theta_fit or refit:
        #     self.theta = self.get_pointestimate(lambd_=lambd_)
        self.sigma = self.compute_sigma(self.lambd_)
        return self.theta, self.sigma

    def compute_sigma(self, lambd):
    ...
    def get_lambda(self, lambd_0=0.5, idx_to_optimize=None):
